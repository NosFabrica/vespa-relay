...
node tools/favicon/build.mjs       # regenerate the tab icon (also plain node)
...
relay/src/main/resources/
  ...
  web/favicon.{svg,ico}
                        the tab icon, GENERATED by tools/favicon/build.mjs from
                        index.html's own brand mark and committed — nothing in
                        the build runs node, so the generator cannot be the
                        check and `RelayFaviconTest` is: it compares the two
                        drawings circle by circle and fails if either moves
                        alone. The favicon is deliberately NOT the mark
                        verbatim. The mark is `currentColor` at 1.7-unit
                        strokes, and a tab strip has no `currentColor` to
                        inherit and comes in both themes, so the icon is white
                        ink on an accent tile with a heavier stroke and less
                        margin. All three pages link both formats (SVG first,
                        .ico as the fallback that also answers the guessed
                        /favicon.ico — see `favicon()` in HttpServer)
