...
| `TRUST_RECONCILE_ON_START` | settle the trust projection before the relay serves. It is a **barrier**: no websocket listener and no router until it finishes, which on a large corpus can be many minutes. `false` serves immediately and leaves ranked search unsettled | `true` |
