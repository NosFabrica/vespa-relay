...
| `ROUTER_MONITOR_NSEC` | NIP-66 monitor identity. The router remembers relay liveness as kind 30166 in its own store, TTL'd at 24h so a dead mark is "not now", never "never again". **Reading is always on and needs no key** — 30166s from other monitors are ordinary events, so adding kind 30166 to a stream's filter inherits the network's census without dialling anything. This key only publishes our own findings back. Deliberately separate from `ROUTER_AUTH_NSEC`: monitoring and authenticating are different claims | unset ⇒ read-only |
