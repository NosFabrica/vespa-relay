// The last ranked ask, kept so the next view reuses it instead of asking the
// relay the same question again. The relay's cost for a NIP-50 search is the
// match set, not the page, and the type-ahead and the results view ask the
// same question at one width (paging.js's askLimit). One entry, keyed by
// everything the filters say except the limit, reused only when the limit
// matches too. A cache of promises, since Enter usually lands while the
// popup's ask is still in flight; a rejected promise is dropped so a failure
// is retried.
...
/**
 * Everything about [filters] that decides the answer, minus the limit. Keys
 * are sorted because field order inside a filter does not matter to a relay.
 */
...
export class AskCache {
  ...
  take(filters, ask) {
    ...
    // take() this exists to fold.
    let promise;
    try { promise = Promise.resolve(ask()); } catch (e) { promise = Promise.reject(e); }
    const entry = { key, limit, at, promise };
    this.last = entry;
    // A failed ask is not kept, and neither is a short one: a read the page
    // stopped listening to resolves with `complete: false` (shared/relay.js),
    // and the next take() must ask again rather than draw an unfinished page.
