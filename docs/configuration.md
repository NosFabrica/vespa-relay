...
| `GUARD_OWNERS_DISABLE` | **not a setting — a tripwire.** It turns off the store's per-instance cache of "authors known to have a kind 5/62", so NIP-09 and NIP-62 are checked against the store on *every* insert. Required here because two processes write one Vespa and that cache is per instance: deletions the sync process mirrors ([Mirroring the deletions themselves](router.md#mirroring-the-deletions-themselves)) are invisible to the relay's copy, so either one would re-admit what the other erased. The image and the Gradle `run` tasks set it, and **both entrypoints refuse to boot without it** | `1`, enforced |
