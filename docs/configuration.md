...
- **sync** — present only where a router writes beside this relay. The
  *coverage* half is how far the mirror has walked each relay (`streams`, from
  ...
  are a **partition**: `discovered = foldedOntoAnother + taken`, and the eight
  outcomes under `taken` (including a derived `pending`) sum to `taken` while the
  cycle is still running. `staleForSec` is the heartbeat — how long the router
