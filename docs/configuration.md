...
| `MONITOR_STATUS_PORT` | where the MONITOR plane serves **its own status page** — the round-up that finds candidate urls, the alias fold, the consistency gate and the fitness grades, plus a panel that reads the signed kind-30166 records themselves out of the relay over its own websocket. Its own page rather than a panel on the mirror's, because it answers a different question in a different unit: sync coverage is measured in EVENTS and asks whether the mirror is keeping up, this is measured in RELAY URLS and asks which of them may be dialled at all. It is also what the eventual process split needs — when this plane moves into its own container, the port it is read at does not change. The document publishes the served relay's ws url so the verdict panel knows what to dial: this page is not served by the relay, and deriving the socket from `location` would open one against the status site. Set to 0 to serve no monitor page | `7779` |
