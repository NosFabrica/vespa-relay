...
| `RELAY_AUTHED_TOKEN` | a shared secret that turns on `GET /authed` — **who holds a verified NIP-42 login right now** — so the sync process can scope a stream's `relaySource` to the people currently reading — holding a live subscription on each of their own outbox and inbox relays, and on their trust provider, for as long as they are here (see [Following the people who are signed in](router.md#following-the-people-who-are-signed-in)). This is the one document this relay serves that names its **clients**: every other public endpoint here describes stored events or an aggregate, and `/pressure` caps its `samples` field precisely so that polling it cannot become a client-activity feed. So the route exists **only** where this is set, every request must carry `Authorization: Bearer <it>`, and a missing, malformed or wrong credential gets one 401 with one body. Set the same value as `SYNC_AUTHED_TOKEN` on the sync service. Unset ⇒ no route, and nothing about logins is tracked at all | unset ⇒ off |
...
- **sync** — present only where a router writes beside this relay. The
  *coverage* half is how far the mirror has walked each relay (`streams`, from
  ...
  timestamp: this file outlives the process that wrote it. The *`progress`* half
  is what the router is **doing** — per stream, its phase and the disposition of
  every url the current pass took on, from `SYNC_PROGRESS_FILE`. Those counts
  are a **partition**: `discovered = foldedOntoAnother + excluded + taken`, and
  ...
  past a few minutes means the sync process is not running. `terms` defines every
  number in the section, because a count nobody can read without the source is
  not published
...
| `SYNC_AUTHED_URL` / `SYNC_AUTHED_TOKEN` | the relay's `GET /authed` endpoint and the secret it gates it with (its `RELAY_AUTHED_TOKEN`), polled every 10s into the relay list of every `presence { }` stream. **Required by any such stream**: one declared with this unset would hold zero subscriptions on an empty relay list forever, which from outside is indistinguishable from a night nobody signed in — so the sync process refuses to boot instead, naming the streams. A url with no token is refused for the same reason: it could only ever collect 401s. After ~30s of failed polls the set is **emptied** rather than held (the sockets those readers were signed in on live in the relay we cannot reach) and the log says so; a 401 is reported in its own words, because it is fixed in a config file rather than on the network | unset ⇒ presence streams cannot be configured |
...
| `SYNC_PROGRESS_FILE` | where the router writes **what it is doing** — one entry per stream with its phase, how long it has been in it, and the disposition of every url the current cycle took on. State, not config, so it is rewritten on every progress tick rather than once at boot; and its `writtenAt` is a **heartbeat**, advancing whatever the streams are doing, which is the only thing on the relay's side of the volume that can tell a quiet mirror from a stopped one. The url counts are a partition that sums to the number discovered — `discovered = foldedOntoAnother + excluded + taken`, and the ten outcomes under `taken` sum to `taken`, with `pending` derived so it closes mid-pass. Each outcome states its own retry policy, which is what `skipped as dead` never did. Beside the cycle, `inFlight` NAMES the relays a stream has workers on, longest-held first, with what each has delivered and how long it has been quiet — the counts alone could not say which relay was holding a slot, and a stalled leg earns no band, so it appears nowhere else. A stream may also report the phase `holding`: its refresh interval came round and it declined to start the next pass because fewer than half its transfer slots were free. **Read by the relay** on the same terms as the other three, and published as `sync.progress`. Unset ⇒ nothing is published, the router says so on boot, and a cycle that failed reads exactly like one that finished | compose: `/var/lib/vespa-relay/sync-progress.json`; bare: unset |
