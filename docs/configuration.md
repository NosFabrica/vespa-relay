...
| `SYNC_SWEEP_CURSOR_STALE_AFTER_SECONDS` | how old an INTERRUPTED sweep's resume cursor may be and still be resumed from; past it the sweep starts over. Costs at most one sweep either way, which is why it keeps a default. NOT how often a whole history is re-read — that is `refetchThePastSeconds` per stream in `router.conf`, and the env names the two used to share are refused at boot | `604800` (7 days) |
