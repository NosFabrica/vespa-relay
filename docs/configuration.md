...
| `SYNC_REFETCH_THE_PAST_SECONDS` (was `SYNC_FULL_RESYNC_SECONDS`, still accepted) | how long a recorded sync window may narrow work before the router walks the whole filter again — the default for streams that do not set their own `refetchThePastSeconds` (see [the router doc](router.md#refetchthepastseconds-and-the-audit)). **Unset means never**: re-reading a whole history is the most expensive thing here that runs on a clock, so it is written down or it does not run, and any stream left with neither this nor `negentropySyncThePastSeconds` is named at boot | unset — no re-fetch |
