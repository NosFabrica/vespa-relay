...
| `GUARD_OWNERS_DISABLE` | probe NIP-09/NIP-62 on **every** insert, instead of only for authors this process has seen store a kind 5/62. Read by the store, not by this relay. It matters here because the two processes are two writers with two copies of that set: deletions the sync process mirrors ([Mirroring the deletions themselves](router.md#mirroring-the-deletions-themselves)) are invisible to the relay's copy until it restarts, so a client could republish an event a mirrored deletion covers. Compose ships it **on for the relay**, where the cost is a couple of extra queries per client publish; on the sync process the same probes land on every ingest batch under the writer lock, so it is off by default there | `1` on the relay, unset ⇒ off elsewhere |
...
| `SYNC_SWEEP_STATE_FILE` | where the per-peer window size (and the cap a peer stated in a rejection) and the in-progress sweep cursor are kept. Distinct from `SYNC_STATE_FILE`: bands are the long-lived record of what a relay has given us, this is working state a finished sweep throws away. Unset, both are re-learned on every boot — correct, but it pays the sizing ladder again and restarts a partial sweep from the top | unset ⇒ in memory only |
