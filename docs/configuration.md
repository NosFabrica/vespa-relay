...
| var | meaning | default |
|---|---|---|
| `SYNC_CONFIG` | the router `streams { }` config, inline (HOCON). **Required** (or `SYNC_CONFIG_FILE`): a sync process with nothing to sync refuses to start rather than idle in a way that reads as "syncing" | — |
...
| `SYNC_DYNAMIC_REFRESH_SECONDS` | default period between **rediscoveries** of a `relaySource = [...]` stream — how often its relay list is read out of the store again | `21600` (6h) |
