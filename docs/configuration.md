...
| `SYNC_STATE_FILE` | where the per-(relay, filter) synced `created_at` band is kept. A relay without NIP-77 has no memory of what it already sent, so without this every restart re-downloads its whole corpus; with it the router asks only for what falls outside the band it already walked. Keyed by filter — edit a stream's filter and that stream starts over. **Also read by the relay**, off the shared `/var/lib/vespa-relay` mount, for the Sync coverage card on `/stats.html`: set the same path on both services (the compose default already matches) or the card charts where the router used to write. Read-only there — the router stays the only writer | unset ⇒ in memory only |
