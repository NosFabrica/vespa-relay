...
| `MONITOR_CONFIG_FILE` | path to a file holding that config, as an alternative to `MONITOR_CONFIG`. Under compose it is set **only** when `MONITOR_CONFIG_LOCAL` names a file of yours: the mount always has something at the path, and defaulting to it would start signing public claims about strangers' relays for a deployment that never asked | compose: `/etc/vespa-relay/monitor.conf` when `MONITOR_CONFIG_LOCAL` is set, **empty** otherwise |
| `MONITOR_CONFIG_LOCAL` | compose only: the **host** path mounted at `/etc/vespa-relay/monitor.conf`, and the opt-in that sets `MONITOR_CONFIG_FILE`. Unset ⇒ the monitor reads nothing and measures nothing, and a `sync.conf` whose streams discover their relays refuses to boot until you point this at your copy | unset (the mount falls back to `./monitor.conf.example`, which nothing then reads) |
